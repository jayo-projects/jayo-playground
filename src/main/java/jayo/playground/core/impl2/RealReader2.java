...
import static jayo.playground.core.impl2.ReaderSegmentQueue.newReaderSegmentQueue;
import static jayo.playground.core.impl2.ReaderSegmentQueue.newSyncReaderSegmentQueue;

public final class RealReader2 implements Reader {
    private static final long INTEGER_MAX_PLUS_1 = (long) Integer.MAX_VALUE + 1;

    final @NonNull ReaderSegmentQueue segmentQueue;

    RealReader2(final @NonNull RawReader reader) {
        assert reader != null;
        segmentQueue = newSyncReaderSegmentQueue(reader);
    }

    public RealReader2(final @NonNull RawReader reader, final @Nullable TaskRunner taskRunner) {
        assert reader != null;
