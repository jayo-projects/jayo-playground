...
public final class RealBuffer3 implements Buffer {
    ...
    final @NonNull BasicFifoQueue<@NonNull Segment> segmentQueue;
    long byteSize = 0L;

    public RealBuffer3() {
        this(BasicFifoQueue.create());
    }

    RealBuffer3(final @NonNull BasicFifoQueue<@NonNull Segment> segmentQueue) {
    ...
    private void writeUtf16ToUtf8(final @NonNull CharSequence charSequence,
        ...
        while (i < endIndex) {
            ...
            if (c < 0x80) {
                final var tail = writableSegment(1);
                ...
                final var tail = writableSegment(2);
                final var tailByteBuffer = tail.byteBuffer;
                final var limit = tailByteBuffer.limit();
    ...
    @NonNull Segment writableSegment(final int minimumCapacity) {
        final var tail = segmentQueue.peekLast();
