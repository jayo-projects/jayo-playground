...
sealed abstract class TaskQueue6<T extends Task6<T>> implements TaskQueue {
    final @NonNull TaskRunner6 taskRunner;
    ...
    /**
     * This queue's currently waiting for execution task in the {@link TaskRunner6}, or null if no future tasks.
     */
    @Nullable
    ...
    TaskQueue6(final @NonNull TaskRunner6 taskRunner,
               final @NonNull String name,
    ...
    static final class ScheduledQueue extends TaskQueue6<Task6.ScheduledTask> implements ScheduledTaskQueue {
        /**
         * True if the {@link #activeTask} should be canceled when it completes.
         */
        boolean cancelActiveTask = false;

        ScheduledQueue(final @NonNull TaskRunner6 taskRunner, final @NonNull String name) {
        ...
        static final class AwaitIdleTask extends Task6.ScheduledTask {
         ...
         */
        private void schedule(final Task6.@NonNull ScheduledTask task, final long delayNanos) {
            assert task != null;
            assert delayNanos >= 0;
        ...
        boolean scheduleAndDecide(final Task6.@NonNull ScheduledTask task, final long delayNanos) {
            assert task != null;
    ...
    static final class RunnableQueue extends TaskQueue6<Task6.RunnableTask> {
        RunnableQueue(final @NonNull TaskRunner6 taskRunner, final @NonNull String name) {
        ...
        static final class AwaitIdleTask extends Task6.RunnableTask {
            private final @NonNull CountDownLatch latch = new CountDownLatch(1);
         ...
         * @throws RejectedExecutionException if the queue is shut down and the task is not cancelable.
         */
        private void schedule(final Task6.@NonNull RunnableTask task) {
            assert task != null;
        ...
        private boolean scheduleAndDecide(final Task6.@NonNull RunnableTask task) {
            assert task != null;
