 ...
 * This applies to both executions scheduled with {@link TaskQueue5#schedule(String, long, LongSupplier)} and those
 ...
 * <h3>Task Queues</h3>
 * Tasks are bound to the {@link TaskQueue5} they are scheduled in. Each queue is sequential and the tasks within it
 * never execute concurrently. It is an error to use a task in multiple queues.
 */
abstract class Task5<T extends Task5<T>> {
    final @NonNull String name;
    final boolean cancellable;

    // Guarded by the TaskRunner.
    @Nullable
    TaskQueue5<T> queue = null;

    Task5(final @NonNull String name, final boolean cancellable) {
    ...
    final void initQueue(final @NonNull TaskQueue5<T> queue) {
        assert queue != null;
    ...
    static abstract class RunnableTask extends Task5<RunnableTask> implements Runnable {
    ...
    static abstract class ScheduledTask extends Task5<ScheduledTask> implements Comparable<ScheduledTask> {
        /**
         * Undefined unless this is in {@link TaskQueue5#futureTasks}.
         */
