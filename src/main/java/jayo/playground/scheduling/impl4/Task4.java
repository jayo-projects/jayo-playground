 ...
 * This applies to both executions scheduled with {@link TaskQueue4#schedule(String, long, LongSupplier)} and those
 ...
 * <h3>Task Queues</h3>
 * Tasks are bound to the {@link TaskQueue4} they are scheduled in. Each queue is sequential and the tasks within it
 * never execute concurrently. It is an error to use a task in multiple queues.
 */
abstract class Task4<T extends Task4<T>> {
    final @NonNull String name;
    final boolean cancellable;

    // Guarded by the TaskRunner.
    @Nullable
    TaskQueue4<T> queue = null;

    Task4(final @NonNull String name, final boolean cancellable) {
    ...
    final void initQueue(final @NonNull TaskQueue4<T> queue) {
        assert queue != null;
    ...
    static abstract class RunnableTask extends Task4<RunnableTask> implements Runnable {
    ...
    static abstract class ScheduledTask extends Task4<ScheduledTask> implements Comparable<ScheduledTask> {
        /**
         * Undefined unless this is in {@link TaskQueue4#futureTasks}.
         */
