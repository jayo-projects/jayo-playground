 ...
 * This applies to both executions scheduled with {@link TaskQueue2#schedule(String, long, LongSupplier)} and those
 ...
 * <h3>Task Queues</h3>
 * Tasks are bound to the {@link TaskQueue2} they are scheduled in. Each queue is sequential and the tasks within it
 * never execute concurrently. It is an error to use a task in multiple queues.
 */
abstract class Task2 implements Comparable<Task2> {
    final @NonNull String name;
    final boolean cancellable;

    // Guarded by the TaskRunner.
    @Nullable
    TaskQueue2 queue = null;

    /**
     * Undefined unless this is in {@link TaskQueue2#futureTasks}.
     */
    long nextExecuteNanoTime = -1L;

    Task2(final @NonNull String name, final boolean cancellable) {
    ...
    final void initQueue(final @NonNull TaskQueue2 queue) {
        assert queue != null;
    ...
    @Override
    public final int compareTo(final @NonNull Task2 other) {
