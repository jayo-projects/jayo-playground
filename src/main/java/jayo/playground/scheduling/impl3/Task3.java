 ...
 * This applies to both executions scheduled with {@link TaskQueue3#schedule(String, long, LongSupplier)} and those
 ...
 * <h3>Task Queues</h3>
 * Tasks are bound to the {@link TaskQueue3} they are scheduled in. Each queue is sequential and the tasks within it
 * never execute concurrently. It is an error to use a task in multiple queues.
 */
abstract class Task3<T extends Task3<T>> {
    final @NonNull String name;
    final boolean cancellable;

    // Guarded by the TaskRunner.
    @Nullable
    TaskQueue3<T> queue = null;

    Task3(final @NonNull String name, final boolean cancellable) {
    ...
    final void initQueue(final @NonNull TaskQueue3<T> queue) {
        assert queue != null;
    ...
    static abstract class RunnableTask extends Task3<RunnableTask> implements Runnable {
    ...
    static abstract class ScheduledTask extends Task3<ScheduledTask> implements Comparable<ScheduledTask> {
        /**
         * Undefined unless this is in {@link TaskQueue3#futureTasks}.
         */
