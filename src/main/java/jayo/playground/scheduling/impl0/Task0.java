 ...
 * This applies to both executions scheduled with {@link TaskQueue0#schedule} and those implied by the returned execution
 * delay.
 * <h3>Cancellation</h3>
 ...
 * <h3>Task Queues</h3>
 * Tasks are bound to the {@link TaskQueue0} they are scheduled in. Each queue is sequential and the tasks within it
 * never execute concurrently. It is an error to use a task in multiple queues.
 */
abstract class Task0 {
    final @NonNull String name;
    final boolean cancellable;

    // Guarded by the TaskRunner.
    @Nullable
    TaskQueue0 queue = null;

    /**
     * Undefined unless this is in {@link TaskQueue0#futureTasks}.
     */
    long nextExecuteNanoTime = -1L;

    Task0(final @NonNull String name, final boolean cancellable) {
    ...
    final void initQueue(final @NonNull TaskQueue0 queue) {
        assert queue != null;
