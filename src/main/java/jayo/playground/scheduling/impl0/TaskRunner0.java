...
public final class TaskRunner0 implements TaskRunner {
    ...
    /**
     * Queues with tasks that are currently executing their {@link TaskQueue0#activeTask}.
     */
    private final List<TaskQueue0> busyQueues = new ArrayList<>();

    /**
     * Queues not in {@link #busyQueues} that have non-empty {@link TaskQueue0#futureTasks}.
     */
    private final List<TaskQueue0> readyQueues = new ArrayList<>();

    public TaskRunner0(final @NonNull ExecutorService executor) {
    ...
    void kickCoordinator(final @NonNull TaskQueue0 taskQueue) {
    ...
    private void beforeRun(final @NonNull Task0 task) {
        assert task != null;
    ...
    private void afterRun(final @NonNull Task0 task,
     ...
     */
    private @Nullable Task0 awaitTaskToRun() {
        while (true) {
            ...
            for (final var queue : readyQueues) {
                final var candidate = queue.futureTasks.get(0);
    ...
    public @NonNull ScheduledTaskQueue newScheduledQueue() {
        ...
        return new TaskQueue0(this, "Q" + name);
    }
