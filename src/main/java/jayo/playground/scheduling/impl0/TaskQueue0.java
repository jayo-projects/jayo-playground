...
package jayo.playground.scheduling.impl0;
...
final class TaskQueue0 implements ScheduledTaskQueue {
    final @NonNull TaskRunner0 taskRunner;
    final @NonNull String name;

    boolean shutdown = false;

    /**
     * Scheduled tasks ordered by {@link Task0#nextExecuteNanoTime}.
     */
    @NonNull
    final List<Task0> futureTasks = new ArrayList<>();

    /**
     * This queue's currently-executing task, or null if none is currently executing.
     */
    @Nullable
    Task0 activeTask = null;
    ...
    TaskQueue0(final @NonNull TaskRunner0 taskRunner, final @NonNull String name) {
    ...
    public void execute(final @NonNull String name, final boolean cancellable, final @NonNull Runnable block) {
        ...
        schedule(new Task0(name, cancellable) {
                     @Override
                     protected long runOnce() {
                         block.run();
                         return -1L;
     ...
     */
    private void schedule(final @NonNull Task0 task, final long delayNanos) {
        assert task != null;
        assert delayNanos >= 0;
    ...
    private static final class AwaitIdleTask extends Task0 {
    ...
    boolean scheduleAndDecide(final @NonNull Task0 task, final long delayNanos) {
        assert task != null;
        ...
        final var existingIndex = futureTasks.indexOf(task);
        if (existingIndex != -1) {
            if (task.nextExecuteNanoTime <= executeNanoTime) {
                return false;
            }
            futureTasks.remove(existingIndex); // Already scheduled later: reschedule below!
        }
        ...
        var insertAt = indexOfFirst(futureTasks, now, delayNanos);
        if (insertAt == -1) {
    ...
    private int indexOfFirst(@NonNull List<Task0> futureTasks, long now, long delayNanos) {
        var index = 0;
        for (final var futureTask : futureTasks) {
            if (futureTask.nextExecuteNanoTime - now > delayNanos) {
                return index;
            }
            index++;
        }
    ...
    boolean cancelAllAndDecide() {
        ...
        for (var i = futureTasks.size() - 1; i >= 0; i--) {
            if (futureTasks.get(i).cancellable) {
                tasksCanceled = true;
